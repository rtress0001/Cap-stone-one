...
#Test for critical interferance

#Creating a filter for depresion anxiety to make to make a new mean
summary_data = stress[stress["depression"] > 19]
summary_data = summary_data[summary_data["anxiety_level"]>19]
summary_data["academic_performance"].mean()


#Creating a filter for self esteem and anxiety to make a new mean
summary_data1 = stress[stress["self_esteem"] < 11]
summary_data1 = summary_data1[summary_data1["anxiety_level"]>19]
summary_data1["academic_performance"].mean()

#creating a filter for self esteem and mental health history
summary_data2 = stress[stress["self_esteem"] < 11]
summary_data2 = summary_data2[summary_data2["mental_health_history"]==1]
summary_data2["academic_performance"].mean()

#Testing for mental health minimum.
summary_data = stress.groupby(['self_esteem', 'mental_health_history'])['academic_performance'].mean().reset_index()

# Create a scatter plot
plt.figure(figsize=(8, 6))
sns.lmplot(data=summary_data, x='self_esteem', y='academic_performance', hue='mental_health_history', palette='Dark2')
plt.title("Line Plot of Average Academic Performance by depression")
plt.xlabel("self esteem")
plt.ylabel("Mean Academic Performance")
plt.show()


#Creating a group by for depresion
summary_data = stress.groupby(['depression', 'mental_health_history'])['academic_performance'].mean().reset_index()
...
#creating a group by for headahce
summary_data = stress.groupby(['headache', 'mental_health_history'])['academic_performance'].mean().reset_index()

# Create a scatter plot
plt.figure(figsize=(8, 6))
sns.lmplot(data=summary_data, x='headache', y='academic_performance', hue='mental_health_history', palette='Dark2')
plt.title("Line Plot of Average Academic Performance by depression")
plt.xlabel("headache")
plt.ylabel("Mean Academic Performance")
plt.show()

#creating a group by for anxiety
summary_data = stress.groupby(['anxiety_level', 'mental_health_history'])['academic_performance'].mean().reset_index()
...
summary_data = stress.groupby(['sleep_quality', 'mental_health_history'])['academic_performance'].mean().reset_index()

# Create a scatter plot
plt.figure(figsize=(8, 6))
sns.lmplot(data=summary_data, x='sleep_quality', y='academic_performance', hue='mental_health_history', palette='Dark2')
plt.title("Line Plot of Average Academic Performance by sleep quality")
plt.xlabel("sleep quality")
plt.ylabel("Mean Academic Performance")
plt.show()

summary_data = stress.groupby(['noise_level', 'mental_health_history'])['academic_performance'].mean().reset_index()

# Create a stater plot and a line plot
plt.figure(figsize=(8, 6))
sns.lmplot(data=summary_data, x='noise_level', y='academic_performance', hue='mental_health_history', palette='Dark2')
plt.title("Line Plot of Average Academic Performance by noise level")
